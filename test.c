...
void run_test(const char *filename) {
    ...
    snprintf(compressed_path, sizeof(compressed_path), "%s/%s.huf", OUTPUT_DIR, filename);
    snprintf(decompressed_path, sizeof(decompressed_path), "%s/%s", OUTPUT_DIR, filename);
...
int main(void) {
    ...
    struct dirent *dir;
    // Collect files first to sort them (readdir order is undefined)
    // For simplicity, we'll just process them as they come, but skip non-test files
    // A real implementation might sort them.
